...
public class Settings : MonoBehaviour
{
    ...
    void Start()
    { 
         // Set up PlayerInputManager event handlers
    ...
    {
        // Add the new player GameObject
        AddPlayer(playerInput.gameObject);

        // UI for the new player
        int playerIndex = players.Count - 1;
        if (playerIndex < playerUIList.Count)
        {
            var ui = PlayerUI(playerIndex);
            if (ui != null)
            {
                ui.SetActive(true);
            }
    ...
    {
        // Remove the player GameObject
        RemovePlayer(playerInput.gameObject);

        // Disable the UI for the player
        int playerIndex = GetPlayerIndex(playerInput.gameObject);
        if (playerIndex >= 0 && playerIndex < playerUIList.Count)
        {
            var ui = PlayerUI(playerIndex);
            if (ui != null)
            {
